...
def parse_args():
    parser = argparse.ArgumentParser(description='Script for uploading or updating existing file on Google Drive.'
                                                 'Credentials should be passed stored in "credentials.json" file.')
    parser.add_argument('--folder_id', required=True, help='ID of the folder that Service Account has access to, '
                                                           'and where the file will be stored')
...
def authenticate() -> GoogleDrive:
    ...
    try:
        ...
        gauth.credentials = ServiceAccountCredentials.from_json_keyfile_name('credentials.json',
...
def get_file(drive: GoogleDrive, folder_id: str, file_name: str) -> GoogleDriveFile:
    ...
    if len(file_list) == 1:
        ...
        my_file = drive.CreateFile(metadata={
